...
include "functions.php";
	...
	if($_SESSION['sort_by']=="date")
		...
		$page=1;//Default page
		$limit=posts_per_page();//Records per page
		$next=2;
		$prev=1;
...
?>
	...
	<title>Videos - <?php echo('Page ' . $page) ?></title>
    ...
    <div id="content-holder"> 
        ...
        <div class="main-filter ">
            <ul class="content-type">
					...
					<?php } if(allow_videos()) { ?>
						<li> <a class="current" href="<?php echo(rootpath()) ?>/videos"><strong>Videos</strong></a></li>
						<?php if(rss_enable() && rss_cat_enable()) { ?>
						<li><a class="" href="<?php echo(rootpath()) ?>/rss/videos" style="padding-left: 8px; padding-right: 11px;"><img src="<?php echo(rootpath()) ?>/images/rss_icon.png"  /></a></li>
					<?php }}?>
            </ul>
			...
			<div id="sort">
				<label>
					...
					<select onchange="self.location=self.location='<?php echo(rootpath()) ?>/videos.php?sort_by='+this.options[this.selectedIndex].value">
					...
					<a href="<?php echo(rootpath()) ?>/videos.php?sort_order=DESC"><img src="<?php echo(rootpath()) ?>/images/arrow_up.png" title="Ascending"></a>
					<?php } else if($_SESSION['sort_order']=="DESC") { ?>
					<a href="<?php echo(rootpath()) ?>/videos.php?sort_order=ASC"><img src="<?php echo(rootpath()) ?>/images/arrow_down.png" title="Descending"></a>
					 <?php } else { ?>
					 <a href="<?php echo(rootpath()) ?>/videos.php?sort_order=DESC"><img src="<?php echo(rootpath()) ?>/images/arrow_up.png" title="Ascending"></a>
					<?php } ?>
			</div>
        ...
        <div id="content" listPage="hot">                        
			<div id="entries-content" class="grid"> 
				...
				$sql = "select permalink,title,file,thumb from media where type=1 and approved=1 order by " . $sort_by . " " . $_SESSION['sort_order'] . " limit ".$start_result.",".$limit;
				...
				<ul id="grid-col-1" class="col-<?php echo($i) ?>">
                    <li style="height:200px">
                        <a href="<?php echo(rootpath()) ?>/video/<?php echo($fetch['permalink']) ?>" class="jump_stop">
                            <div style="" class="thimage">
							<img style="width: auto;height: 100%;" src="<?php echo(rootpath()) ?>/uploads/thumbs/<?php echo($fetch['thumb']) ?>" alt="<?php echo(ucwords($fetch['title'])) ?>" title="<?php echo(ucwords($fetch['title'])) ?>" />
