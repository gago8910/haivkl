...
include "../functions.php";	
	...
	$error="";
	$invalid=false;
function valid_pic_extension($ext)
{
...
function update_pic($id,$title,$description,$permalink,$file,$thumb,$source,$views,$votes,$date,$approved)
{
	...
	$update_sql = "update media set title='" . $title . "', description='" . $description . "', permalink='" . $permalink . "', file='" . $file . "',thumb='" . $thumb . "',source='" . $source . "',views=" . $views . ",votes=" . $votes . ",date='" . $date . "', orderid=" . $order . ",approved=" . $approved . " where id=" . $id;
	mysql_query($update_sql);
}
if(isset($_GET['rg']) && isset($_GET['id']))
{
	$sql="select file from media where id=" . $_GET['id'] . " and type=0";
	$query = mysql_query($sql);
	$fetch = mysql_fetch_array($query);
	unlink("../uploads/thumbs/" . $fetch['file']);
	regen_pic_thumb($_GET['id'],rootpath() . "/uploads/" . $fetch['file'],false);
}
if(isset($_POST['title']))
{
	$id = $_POST['id'];
	$sql="select title,file,permalink from media where id=" . $id . " and type=0";
	$query = mysql_query($sql);
	$fetch = mysql_fetch_array($query);
	$id = $_POST['id'];
	...
	$thumb = $file;
	if($file!=$fetch['file'] && valid_pic_extension(end(explode(".",$file))))
	{
			if(trim($_FILES["myfile"]["name"])!="")
			{
			...
			$file = regen_pic_thumb($_POST['id'],$file,true);
			$thumb = $file;
			}
			else
			{
	...
	if(!valid_pic_extension(end(explode(".",$file))) && !$file==0)
	$error .="o. Invalid Picture Format<br />";
	...
	$error .="o. Invalid date format.<br />";
		...
		{
			delete_tags($id);
			update_pic($id,$title,$description,$permalink,$file,$thumb,$source,$views,$votes,$date,$approved);
		$tag=trim($_POST['tags']);
	...
	{
		$sql="select * from media where id=" . $_GET['id'] . " and type=0";
		$query = mysql_query($sql);
		$fetch = mysql_fetch_array($query);
		if($fetch['id'])
		{
...
?>
<title>Edit Picture - <?php echo(get_title()) ?></title>
<?php
...
?>      
        <div id="containerHolder">
			<div id="container">
                ...
                <div id="main">
				<br />
				<form action="./edit_pic.php" method="post" enctype="multipart/form-data">
					<fieldset>
					...
					<p><label><b>Picture URL</b></label>
					<input style="width:60%" name="file" value="<?php echo(rootpath() . "/uploads/" . $file) ?>" type="text" class="text-long">‎</p>
					
					<p><label><b>Reupload Picture</b></label>
					<input type="file" id="myfile" name="myfile"/>*Upload from your computer</p>
					
					<p><label><b>Thumbnail</b></label>
					<input style="width:70%" name="thumb" value="<?php echo(rootpath() . "/uploads/thumbs/" . $thumb) ?>" type="text" class="text-long"><a href="./edit_pic.php?id=<?php echo($id) ?>&rg=<?php echo($id) ?>" class="myButton">Regenerate</a></p>
					...
					<a style="padding: 9px 24px;" href="./pictures.php?delete=<?php echo($id) ?>" class="myButton">Delete</a>
					</fieldset>
					</form>
					<?php
					if(isset($_POST['title']) && $error=="" && !$invalid)
					echo('<div class="alert alert-success">Picture Updated Successfully</div>');
					if(isset($_POST['title']) && $error!="")
					echo('<div class="alert alert-error">' . $error . '</div>');
					if(isset($_GET['rg']) && isset($_GET['id']))
					echo('<div class="alert alert-success">Thumbnail Regenerated</div>');
					if($invalid)
					echo('<div class="alert alert-error">Invalid Picture. Not Updated</div>');
					?>
                </div>
                <!-- // #main -->
