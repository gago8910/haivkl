...
if (!isset($_SESSION)) session_start();
	...
	include "header.php";
?>
	<title>Animated GIFs - <?php echo('Page ' . $page) ?></title>
</head>
	<?php
    ...
    <div id="content-holder"> 
        <div class="main-filter ">
            <ul class="content-type">
					...
					<li><a class="" href="<?php echo(rootpath()) ?>"><strong>All</strong></a></li>
					<?php } if(allow_pictures()) { ?>	
						<li> <a class="" href="<?php echo(rootpath()) ?>/pictures"><strong>Pictures</strong></a></li>
					<?php } if(allow_gifs()) { ?>
						<li> <a class="current" href="<?php echo(rootpath()) ?>/animated-gifs"><strong>Animated GIFs</strong></a></li>
						<?php if(rss_enable() && rss_cat_enable()) { ?>
							<li><a class="" href="<?php echo(rootpath()) ?>/rss/gifs" style="padding-left: 8px; padding-right: 11px;"><img src="<?php echo(rootpath()) ?>/images/rss_icon.png" /></a></li>
			...
			<div id="sort">
				<label>
					<strong>Sort by : </strong>
						<select onchange="self.location=self.location='<?php echo(rootpath()) ?>/animated-gifs.php?sort_by='+this.options[this.selectedIndex].value">
				...
				<a href="<?php echo(rootpath()) ?>/animated-gifs.php?sort_order=DESC"><img src="<?php echo(rootpath()) ?>/images/arrow_up.png" title="Ascending"></a>
				<?php } else if($_SESSION['sort_order']=="DESC") { ?>
				<a href="<?php echo(rootpath()) ?>/animated-gifs.php?sort_order=ASC"><img src="<?php echo(rootpath()) ?>/images/arrow_down.png" title="Descending"></a>
				<?php } else { ?>
				<a href="<?php echo(rootpath()) ?>/animated-gifs.php?sort_order=DESC"><img src="<?php echo(rootpath()) ?>/images/arrow_up.png" title="Ascending"></a>
				<?php } ?>
			</div>
        ...
        <div id="content" listPage="hot">                       
            <div id="entries-content" class="grid"> 
				<?php
					...
					$sql = "select permalink,title,file from media where type=2 and approved=1 order by " . $sort_by . " " . $_SESSION['sort_order'] . " limit ".$start_result.",".$limit;
				...
				?>
					<ul id="grid-col-1" class="col-<?php echo($i) ?>">
						<li style="height:200px">
							<a href="<?php echo(rootpath()) ?>/gif/<?php echo($fetch['permalink']) ?>" class="jump_stop">
								<div style="" class="thimage">
									<img style="width: auto;" src="<?php echo(rootpath()) ?>/uploads/thumbs/<?php echo($fetch['file']) ?>" alt="<?php echo(ucwords($fetch['title'])) ?>" title="<?php echo(ucwords($fetch['title'])) ?>" />                            
									
									<div id="title_m"><?php echo(ucwords($fetch['title'])) ?></div></a>
								</div>							
						</li>
					</ul>
				<?php
