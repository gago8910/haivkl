...
if ($page > 1) {
    ?>
    <title><?php echo(get_title() . ' - Page ' . $page) ?></title>
<?php } else { ?>
    ...
    <div id="content-holder">
                    ...
                    <?php if (rss_enable()) { ?>
                        <li><a class="link" href="<?php echo(rootpath()) ?>/rss"
                               style="padding-left: 8px; padding-right: 11px;"><img
                                    src="<?php echo(rootpath()) ?>/images/rss_icon.png"/></a></li>
                    <?php } ?>
                <?php
                }
                if (allow_pictures()) {
        ...
        <div id="content" listPage="hot">
            <div id="entries-content" class="grid">
                ...
                if ((allow_pictures()) && (!allow_videos()) && (!allow_gifs())) {
                    $sql = "select permalink,title,file,thumb,type from media
									where approved=1 and type=0 order by " . $sort_by . " " . $_SESSION['sort_order'] . " limit " . $start_result . "," . $limit;
                }
                if ((!allow_pictures()) && (allow_videos()) && (!allow_gifs())) {
                    $sql = "select permalink,title,file,thumb,type from media
									where approved=1 and type=1 order by " . $sort_by . " " . $_SESSION['sort_order'] . " limit " . $start_result . "," . $limit;
                }
                if ((!allow_videos()) && (!allow_pictures()) && (allow_gifs())) {
                    $sql = "select permalink,title,file,thumb,type from media
									where approved=1 and type=2 order by " . $sort_by . " " . $_SESSION['sort_order'] . " limit " . $start_result . "," . $limit;

                }
                if ((allow_videos()) && (allow_pictures()) && (!allow_gifs())) {
                    $sql = "select permalink,title,file,thumb,type from media
									where approved=1 AND type=0 OR type=1 order by " . $sort_by . " " . $_SESSION['sort_order'] . " limit " . $start_result . "," . $limit;

                }
                if ((!allow_videos()) && (allow_pictures()) && (allow_gifs())) {
                    $sql = "select permalink,title,file,thumb,type from media
									where approved=1 and type=0 OR type=2 order by " . $sort_by . " " . $_SESSION['sort_order'] . " limit " . $start_result . "," . $limit;

                }
                if ((allow_videos()) && (!allow_pictures()) && (allow_gifs())) {
                    $sql = "select permalink,title,file,thumb,type from media
									where approved=1 and type=1 OR type=2 order by " . $sort_by . " " . $_SESSION['sort_order'] . " limit " . $start_result . "," . $limit;
                }
                if ((allow_pictures()) && (allow_videos()) && (allow_gifs())) {
                    $sql = "select permalink,title,file,thumb,type from media
									where approved=1 order by " . $sort_by . " " . $_SESSION['sort_order'] . " limit " . $start_result . "," . $limit;
                }
                        ...
                        <a href="<?php echo(rootpath() . '/' . $type . '/' . $fetch['permalink']) ?>" class="jump_stop">
                            <div style="" class="thimage">
                                <img style="width: auto;"
                                     src="<?php echo(rootpath()) ?>/uploads/thumbs/<?php echo($thumb) ?>"
                                     alt="<?php echo(ucwords($fetch['title'])) ?>"
                                     title="<?php echo(ucwords($fetch['title'])) ?>"/>

                                <div id="title_m">
                                    <center><?php echo(ucwords($fetch['title'])) ?></center>
                                </div>
                        </a>
