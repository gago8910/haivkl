...
?>
    <div id="content-holder">
        <div class="post-info-pad">
            ...
            <p>
                <?php if ($type == 0)
                    echo('<a href="' . rootpath() . '/pictures">Ảnh</a>');
                else if ($type == 1)
                    echo('<a href="' . rootpath() . '/videos">Video</a>');
                else if ($type == 2)
                    echo('<a href="' . rootpath() . '/animated-gifs">GIF</a>');
                ?>
                <span class="seperator">|</span>
                <?php echo($datetime) ?>
                <span class="comment"><fb:comments-count
                        href=<?php echo curPageURL() ?>></fb:comments-count> comments</span>
                <span class="views"><span id="love_count"
                                          votes="<?php echo(return_views($permalink)) ?>"><?php echo(return_views($permalink)) ?>
                        views</span></span>
                <?php if (voting_allowed()) { ?>
                    <span class="loved"><span id="love_count"
                                              votes="<?php echo(return_votes($permalink)) ?>"><?php echo(return_votes($permalink)) ?>
                            likes</span></span>
                <?php } ?>
            </p>
            <ul class="actions">
        ...
        <div class="buttons_p_n">
            ...
            ?>
            <a id="prev_p" class="prev_p" title="<?php echo($post_nav[1]) ?>"
               href="<?php echo(rootpath() . '/' . $post_nav[4] . '/' . $post_nav[0]) ?>"></a>
            <a id="next_p" class="next_p" title="<?php echo($post_nav[3]) ?>"
               href="<?php echo(rootpath() . '/' . $post_nav[5] . '/' . $post_nav[2]) ?>"></a>
        </div>
        ...
        <div id="content">
            ...
            <div class="comment-section">
                ...
                <span class="report-and-source">
					<p>
                        Source : <?php echo($source) ?> &nbsp; (<a style="color:red"
                                                                   href="<?php echo(rootpath()) ?>/contact.php?report=<?php echo(curPageURL()) ?>">Report
                            This</a>)
                    </p>
				</span>
                ...
                <div style="margin-left:10px">
                    <div class="fb-comments" data-href="<?php echo(curPageURL()) ?>" data-colorscheme="light"
                         data-width="720"></div>
                </div>
            </div>
                ...
                <div id="entries-content" class="grid">
                    ...
                    <ul id="grid-col-1" class="col-<?php echo($i) ?>">
                        <li style="height:200px">
                            <a href="<?php echo(rootpath() . '/' . $type . '/' . $fetch['permalink']) ?>"
                               class="jump_stop">
                                <div style="" class="thimage">
                                    <img src="<?php echo(rootpath()) ?>/uploads/thumbs/<?php echo($thumb) ?>"
                                         alt="<?php echo(ucwords($fetch['title'])) ?>"
                                         title="<?php echo(ucwords($fetch['title'])) ?>"/>
